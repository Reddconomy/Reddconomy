...
import webbit_lite.helpers.DateHelper;

public class NettyHttpResponse implements webbit_lite.HttpResponse {
    ...
    @Override
    public NettyHttpResponse write(byte content[]) {
        if (response.isChunked()) {
            ctx.getChannel().write(new DefaultHttpChunk(wrappedBuffer(content)));  
        } else {
            write(copiedBuffer(content));
        }    
        return this;
    }
